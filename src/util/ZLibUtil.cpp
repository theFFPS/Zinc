...
ByteBuffer ZLibUtil::compress(const ByteBuffer& buffer) {
    ...
    int ret = zng_compress2((uint8_t*)compressed.data(), &compressedSize, (const uint8_t*)buffer.getBytes().data(), buffer.size(), Z_DEFAULT_COMPRESSION);
    if (ret != Z_OK) {
...
ByteBuffer ZLibUtil::uncompress(const ByteBuffer& buffer, const size_t& decompressedSize) {
    ...
    int ret = zng_uncompress2((uint8_t*)decompressed.data(), &m_decompressedSize, (const uint8_t*)buffer.getBytes().data(), &sourceLen);
    if (ret != Z_OK) {
